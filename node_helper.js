...
module.exports = NodeHelper.create({
	...
	*/
    updateTimetable: function () {
        var url = "https://api.navitia.io/v1/coverage/" + this.config.coverage + "/journeys?from=" + this.config.departureStationUIC + "&to=" + this.config.arrivalStationUIC + "&datetime=" + moment().toISOString() + "&count=" + this.config.numberDays + "&max_nb_transfers=" + this.config.maxNbTransfers + "&data_freshness=realtime";
    ...
    extractUIC: function (id) {
        const match = id.match(/SNCF:\d{8}/);
            ...
            if (nextTrain !== undefined) {
                ...
                for (var j = 0; j < nextTrain.sections.length; j++) {
                    ...
                    if (ride.mode != "walking") {
                        ...
                        var links = ride.display_informations.links;
                        if (links.length > 0) {
                            for (var l = 0; l < links.length; l++) {
                                if (links[l].type == 'disruption') {
                                    idDisruption = links[l].id;
                                }
                            }
                        }     
                            ...
                            for (var k = 0; k < disruptions.length; k++) {
                                if (disruptions[k].disruption_id == idDisruption) {
                                    ...
                                    for (var l = 0; l < impactedStops.length; l++) {
                                        if (this.extractUIC(impactedStops[l].stop_point.id) && this.extractUIC(impactedStops[l].stop_point.id) === this.extractUIC(this.config.departureStationUIC)) {                                            
                                            disruptionInfo['amended_departure_time'] = moment(ride.departure_date_time).format(this.config.dateFormat);
